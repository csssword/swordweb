...
$extend(SwordPullBigtreeTemplate, {
    /**
	...
	initData:function(em,elData){
        ...
        if(value.contains("code") && value.contains("caption")) {
        	if(value.contains('checkPath')||value.contains(";")){
			 ...
			 if($chk(initData)){
			 	vs.each(function(v) {
			 		var treeData = initData.data;
			 		
			 		var findData = function(data,container){
			 			data.each(function(el){
					 		if((el.code||el.CODE) == v){
					 			container.include((el.caption||el.CAPTION));
					 		}else{
					 			for(var key in el) {
									if ($type(el[key]) == 'array') {
										findData(el[key],container);
										break;
									}
								}
					 		}
			 		...
			 		findData(treeData,Captionvalue);
    ...
    initWidget:function(name, formObj){
        ...
        var tree = pageContainer.create("SwordBigTree");
