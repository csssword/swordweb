...
//					'password' : this._textDataHandler.bind(this),
			...
			,_createRow : function(datas, items) {
				var cellsDom = this.fieldRender.render(items);
				var self = this;
			...
			,_defalutDataHandler : function(rowData, type, name, format) {
...
//				var rowNum = 1 / 1 + 1;
			...
			,_textDataHandler : function(rowData, type, name, format) {
				var value = rowData['tds'][name];
				if (value)
					value = value.value;
				if (!$chk(value))
			...
			,_dateDataHandler : function(rowData, type, name, dataformat,
				...
				if ($chk(showValue)) {
					var dataformat = dataformat || cal.defaultdataformat;
