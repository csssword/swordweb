...
/**
  树 组件
  @class swordweb.widgets.SwordBigTree
  @implements Events
  @implements Options
 */
var SwordBigTree = new Class({
    Implements: [Events, Options]
    ,$family: {name: 'SwordBigTree'}
    ,container:null
    /**
     * 构造器
     */
    ...
    ,options: {
        ...
        ,batchSize:200
        /**
         * 下拉树是否依据id数组，自动定位到对应的层（支持懒渲染与懒加载）
         * @type Boolean
         */
        ,isAutoExtendByIdPath: "false"
    ...
    ,initData:function(data, parent) {
        ...
        if (!$chk(this.options.tid) && !$chk(this.options.ctrl)) {
            ...
            if(jsonData){
            	if(data.dataType){
            		this.options.dataType = data.dataType;
            	}
    ...
    ,build: function(options, parent) {
        ...
        if(this.options.isInitLoadData=="true"){
            ...
            if (this.options.select == "true") {
                if(!this.select){
                    var select = new SwordBigTree.Select(this.options, this, parent);
                    this.select = select;
...
//                    setTimeout(this.select.buildTree.bind(this,[this.container]));
    ...
    ,setHalfOrCheckedList:function(array,isHalfChecked) {
        this.builder.draw.setHalfOrCheckedList(array,isHalfChecked,false);
    }
    /**
     * 添加树节点
    ...
    ,getParent:function(node){
        return this.builder.draw.getParent(node);
    }
    /**
     * 验证是否拥有子节点，使用普通与延迟状态
