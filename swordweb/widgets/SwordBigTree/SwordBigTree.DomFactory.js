...
SwordBigTree.DomFactory = new Class({
    ...
    $family: {name: 'SwordBigTree.DomFactory'}
    /**
     * 请求的路径
     */
    ,props:null
...
SwordBigTree.DomFactory.newInstance = function(obj,events) {
    var instance = null ;
    if ($defined(obj.type) && ("json".test(obj.type.trim(), 'i') || "jsonAptitude".test(obj.type.trim(), 'i'))) {

        instance = new SwordBigTree.JSONDomFactory(obj,events);
    } else {
        instance = new SwordBigTree.XMLDomFactory(obj,events);
    }
            ...
            if ($chk(this.props.tid) || $chk(this.props.ctrl)) {
                ...
                pageContainer.postReq({'req':req,'async':false
                    ,'onSuccess':function(resData) {
                        ...
                        //logger.debug("装载树的数据：" + JSON.encode(data), "SwordBigTree");
                        hash = data;
                ...
                hash = JSON.decode(this.props.dataStr);
