...
SwordBigTree.Select = new Class({
    $family: {name: 'SwordBigTree.Select'}
...
//        			this.initCheckStatus();
    ...
    ,initCheckStatus:function(){
    	...
    	if ($chk(sb.get("value")) && this.swordTree.options.treeType==1) {
			var checkl = [];
			var halfCheck = [];
			var rv = $splat(sb.get('realvalue').split(','));
			var draw = this.swordTree.builder.draw;
			var rootIter = draw.iterPathHash[draw.iterRootKey];
			
			var func = function(nodeIterator,pCodeArray,checkl){
				
    			var pcode = nodeIterator.getAttribute(this.swordTree.options.cascadeSign.id);
    			if(pCodeArray.indexOf(pcode)>-1){
    				nodeIterator.setAttribute(draw.options.checkSign,"true");
    				var h = new Hash();
                	h.set(this.swordTree.options.cascadeSign.id, pcode); 
                	var findNode = this.swordTree.getTreeNode(h);
                    if($chk(findNode)){
...
//        			t.clearCheckedStatus();
			...
			rv.each(function(r){
                ...
                var findNode = this.swordTree.getTreeNode(q);
            	var checkp = sb.get("checkPath");
            	if($chk(checkp)){
            		var pCodeArray = checkp.split("|");
            		
                	pCodeArray.each(function(p){
                		var codeArr = p.split(",");
                		draw.recursionChildNodes(rootIter,func,[codeArr,checkl]);
                		
                	}.bind(this));
            	}
                
                if(checkl.length != 0){
                	this.swordTree.setHalfOrCheckedList(checkl,false);
                }
			}.bind(this));
		}
    }
